...
const COLORS = ["#0088FE", "#00C49F", "#FF8042", "#FFBB28", "#AA336A", "#FF33AA", "#33FFAA"];
...
export default function MedicDashboard() {
    ...
    // Prepare chart data
    const riskData = Object.entries(dashboardData.risk_percentages || {}).map(([key, value]) => ({ name: key, value }));
    const genderData = Object.entries(dashboardData.gender_percentages || {}).map(([key, value]) => ({ name: key, value }));
    const bpData = Object.entries(dashboardData.bp_percentages || {}).map(([key, value]) => ({ name: key, value }));
    const cholData = Object.entries(dashboardData.cholesterol_percentages || {}).map(([key, value]) => ({ name: `Level ${key}`, value }));
    ...
    const chartsContainerStyle = { display: "flex", flexWrap: "wrap", gap: "30px", marginBottom: "30px" };
    const chartStyle = {
        flex: 1,
        minWidth: "350px",
    ...
    return (
        ...
        >
            <h2 style={{ color: "#ffffffff", marginBottom: "20px" }}>Active Users trend</h2>
            ...
            <h3 style={{ color: "#fff", marginBottom: "10px" }}>Latest Evaluations</h3>
                ...
                >
                    <thead>
                        <tr>
                            ...
                            <th style={{ padding: "10px", textAlign: "center" }}>Age (Years)</th>
                            <th style={{ padding: "10px", textAlign: "center" }}>Gender</th>
                            <th style={{ padding: "10px", textAlign: "center" }}>Weight</th>
                            <th style={{ padding: "10px", textAlign: "center" }}>Cholesterol</th>
                    ...
                    <tbody>
                        {dashboardData.latest_evals.map((e) => (
                            <tr key={e.id}>
                                <td style={{ padding: "10px", textAlign: "center" }}>Anonymous</td>
                                <td style={{ padding: "10px", textAlign: "center" }}>{Math.floor(e.age / 365)}</td>
                                <td style={{ padding: "10px", textAlign: "center" }}>{e.gender === 1 ? "Male" : "Female"}</td>
                                <td style={{ padding: "10px", textAlign: "center" }}>{e.weight}</td>
                                <td style={{ padding: "10px", textAlign: "center" }}>{e.cholesterol}</td>
                                <td style={{ padding: "10px", textAlign: "center" }}>{e.ap_hi}/{e.ap_lo}</td>
                                <td style={{ padding: "10px", textAlign: "center" }}>{e.smoke ? "Yes" : "No"}</td>
                                <td style={{ padding: "10px", textAlign: "center" }}>{e.active ? "Yes" : "No"}</td>
                                <td style={{ padding: "10px", textAlign: "center" }}>{(e.risk * 100).toFixed(1)}%</td>
                                <td style={{ padding: "10px", textAlign: "center" }}>{e.date}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
